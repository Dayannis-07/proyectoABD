...
if not st.session_state.conectado:
    ...
    if st.button("Probar Conexión"):
        try:
            ...
            st.session_state.credenciales = {"server": server, "database": database, "username": username, "password": password, "driver": driver}
    ...
    if st.session_state.pagina == "menu":
        ...
        opciones = ["1. Listar tablas e índices",
    ...
    # resultados
        ...
        if st.session_state.opcion_seleccionada == 0:
            try:
                ...
                conn = conectar(creds['server'], creds['database'], creds['username'], creds['password'], creds['driver'])
                cursor = conn.cursor()
                ...
                try:
                    ...
                    mostrar_tabla_con_encabezados(
                        cursor, 
                        """SELECT t.name AS Tabla, COUNT(i.index_id) AS CantidadIndices FROM sys.tables t LEFT JOIN sys.indexes i ON t.object_id = i.object_id AND i.name IS NOT NULL GROUP BY t.name ORDER BY CantidadIndices DESC""", "Cantidad de Índices definidos por Tabla"
                    )
